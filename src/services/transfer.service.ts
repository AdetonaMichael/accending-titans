/**
 * Transfer Service
 * Handles all transfer-related API calls: Acceding Titans transfers, bank transfers, and recipient management
 */
...
class TransferService {
  ...
  /**
   * Verify recipient exists (Acceding Titans user)
   */
  ...
  /**
   * Get recent Acceding Titans transfer recipients (quick selection)
  ...
  async getRecentAscendingTitansRecipients(limit: number = 5): Promise<Recipient[] | null> {
    try {
      const response = (await apiClient.get('/wallet/transfer/recipients/Acceding Titans', {
   ...
   * bank_type: 'Acceding Titans', 'external_bank', or 'all'
  ...
  /**
   * Initiate Acceding Titans-to-Acceding Titans transfer
  ...
  async initiateAscendingTitansTransfer(
    ...
    try {
      ...
      console.error('Error initiating Acceding Titans transfer:', error);
